...
std::uintptr_t Memory::findPattern(const char* moduleName, std::string_view pattern, bool reportNotFound = true) noexcept
{
    static auto id = 0;
    ++id;
    ...
    if(reportNotFound)
        MessageBoxA(NULL, (std::string(skCrypt("Failed to find pattern #")) + std::to_string(id) + '!').c_str(), skCrypt("Medusa.uno"), MB_OK | MB_ICONWARNING);
    return 0;
}
...
Memory::Memory() noexcept
{
    ...
    glowObjectManager = *reinterpret_cast<GlowObjectManager**>(findPattern(CLIENT_DLL, skCrypt("\x0F\x11\x05????\x83\xC8\x01").decrypt()) + 3);
    disablePostProcessing = *reinterpret_cast<bool**>(findPattern(CLIENT_DLL, skCrypt("\x83\xEC\x4C\x80\x3D").decrypt()) + 5);
    loadSky = relativeToAbsolute<decltype(loadSky)>(findPattern(ENGINE_DLL, skCrypt("\xE8????\x84\xC0\x74\x2D\xA1").decrypt()) + 1);
    setClanTag = reinterpret_cast<decltype(setClanTag)>(findPattern(ENGINE_DLL, skCrypt("\x53\x56\x57\x8B\xDA\x8B\xF9\xFF\x15").decrypt()));
    lineGoesThroughSmoke = relativeToAbsolute<decltype(lineGoesThroughSmoke)>(findPattern(CLIENT_DLL, skCrypt("\xE8????\x8B\x4C\x24\x30\x33\xD2").decrypt()) + 1);
    cameraThink = findPattern(CLIENT_DLL, "\x85\xC0\x75\x30\x38\x87");
    getSequenceActivity = reinterpret_cast<decltype(getSequenceActivity)>(findPattern(CLIENT_DLL, skCrypt("\x55\x8B\xEC\x53\x8B\x5D\x08\x56\x8B\xF1\x83").decrypt()));
    isOtherEnemy = relativeToAbsolute<decltype(isOtherEnemy)>(findPattern(CLIENT_DLL, skCrypt("\x8B\xCE\xE8????\x02\xC0").decrypt()) + 3);
    auto temp = reinterpret_cast<std::uintptr_t*>(findPattern(CLIENT_DLL, skCrypt("\xB9????\xE8????\x8B\x5D\x08").decrypt()) + 1);
    hud = *temp;
    findHudElement = relativeToAbsolute<decltype(findHudElement)>(reinterpret_cast<uintptr_t>(temp) + 5);
    clearHudWeapon = relativeToAbsolute<decltype(clearHudWeapon)>(findPattern(CLIENT_DLL, skCrypt("\xE8????\x8B\xF0\xC6\x44\x24??\xC6\x44\x24").decrypt()) + 1);
    itemSystem = relativeToAbsolute<decltype(itemSystem)>(findPattern(CLIENT_DLL, skCrypt("\xE8????\x0F\xB7\x0F").decrypt()) + 1);
    setAbsOrigin = relativeToAbsolute<decltype(setAbsOrigin)>(findPattern(CLIENT_DLL, skCrypt("\xE8????\xEB\x19\x8B\x07").decrypt()) + 1);
    insertIntoTree = findPattern(CLIENT_DLL, skCrypt("\x56\x52\xFF\x50\x18").decrypt()) + 5;
    dispatchSound = reinterpret_cast<int*>(findPattern(ENGINE_DLL, skCrypt("\x74\x0B\xE8????\x8B\x3D").decrypt()) + 3);
    traceToExit = findPattern(CLIENT_DLL, skCrypt("\x55\x8B\xEC\x83\xEC\x4C\xF3\x0F\x10\x75").decrypt());
    viewRender = **reinterpret_cast<ViewRender***>(findPattern(CLIENT_DLL, skCrypt("\x8B\x0D????\xFF\x75\x0C\x8B\x45\x08").decrypt()) + 2);
    viewRenderBeams = *reinterpret_cast<ViewRenderBeams**>(findPattern(CLIENT_DLL, skCrypt("\xB9????\x0F\x11\x44\x24?\xC7\x44\x24?????\xF3\x0F\x10\x84\x24").decrypt()) + 1);
    drawScreenEffectMaterial = relativeToAbsolute<uintptr_t>(findPattern(CLIENT_DLL, "\xE8????\x83\xC4\x0C\x8D\x4D\xF8") + 1);
    submitReportFunction = findPattern(CLIENT_DLL, skCrypt("\x55\x8B\xEC\x83\xE4\xF8\x83\xEC\x28\x8B\x4D\x08").decrypt());
    ...
    vignette = *reinterpret_cast<float**>(findPattern(CLIENT_DLL, skCrypt("\x0F\x11\x05????\xF3\x0F\x7E\x87").decrypt()) + 3) + 1;
    ...
    glowBox = findPattern(CLIENT_DLL, "\x55\x8B\xEC\x53\x56\x8D\x59");
    // Custom
    ...
    viewmodelInScope = findPattern(CLIENT_DLL, "\x55\x8B\xEC\x51\x57\xE8");
