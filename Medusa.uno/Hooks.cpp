...
static LRESULT __stdcall wndProc(HWND window, UINT msg, WPARAM wParam, LPARAM lParam) noexcept
{
    ...
    if (gui->isOpen())
    {
        ...
        auto pressed_menu_key = msg == WM_LBUTTONDOWN || msg == WM_LBUTTONUP || msg == WM_RBUTTONDOWN || msg == WM_RBUTTONUP || msg == WM_MOUSEWHEEL;
...
static void __stdcall paintTraverse(unsigned int panel, bool forceRepaint, bool allowForce) noexcept
{
    ...
    if (reload_fonts())
    {
        hooks->IndFont = interfaces->surface->createFont(); interfaces->surface->setFontGlyphSet(hooks->IndFont, c_xor("Calibri"), 30, 610, 0, 0, 0x010, 0, 0);
        hooks->IndShadow = interfaces->surface->createFont();  interfaces->surface->setFontGlyphSet(hooks->IndShadow, c_xor("Calibri"), 30, 610, 0, 0, 0x010, 0, 0);
        hooks->tahomaBoldAA = interfaces->surface->createFont(); interfaces->surface->setFontGlyphSet(hooks->tahomaBoldAA, c_xor("Tahoma"), 12, 500, 0, 0, 0x010 | 0x080, 0, 0);
        hooks->nameEsp = interfaces->surface->createFont(); interfaces->surface->setFontGlyphSet(hooks->nameEsp, c_xor("Verdana"), 12, FW_MEDIUM, 0, 0, fontflag_dropshadow, 0x20, 0xa69f);
        hooks->smallFonts = interfaces->surface->createFont(); interfaces->surface->setFontGlyphSet(hooks->smallFonts, c_xor("Small Fonts"), 9, 500, 0, 0, 0x200);
        hooks->tahomaAA = interfaces->surface->createFont(); interfaces->surface->setFontGlyphSet(hooks->tahomaAA, c_xor("Tahoma"), 12, 400, 0, 0, 0x010 | 0x080, 0, 0);
        hooks->tahomaNoShadowAA = interfaces->surface->createFont(); interfaces->surface->setFontGlyphSet(hooks->tahomaNoShadowAA, c_xor("Tahoma"), 12, 400, 0, 0, 0x010, 0, 0);
        hooks->verdanaExtraBoldAA = interfaces->surface->createFont(); interfaces->surface->setFontGlyphSet(hooks->verdanaExtraBoldAA, c_xor("Verdana"), 25, 800, 0, 0, 0x010 | 0x080, 0, 0);
        hooks->loggerFont = interfaces->surface->createFont(); interfaces->surface->setFontGlyphSet(hooks->loggerFont, c_xor("Consolas"), 10, 400, 0, 0, 0x080, 0, 0);
        AddFontMemResourceEx(weaponIcon, 47320, nullptr, &hooks->weaponIcon);
        hooks->weaponIcon = interfaces->surface->createFont(); interfaces->surface->setFontGlyphSet(hooks->weaponIcon, c_xor("undefeated"), 12, 100, 0, 0, 0x010 | 0x080, 0, 0);
        RemoveFontMemResourceEx(&hooks->weaponIcon);
    }

    if (interfaces->panel->getName(panel) == c_xor("MatSystemTopPanel")) {
        Misc::drawAutoPeekD();
        Visuals::aaLines();
        Logger::process();
        Misc::Indictators();
...
BOOL CALLBACK enumwindowsproc(HWND hwnd1, LPARAM lparam) {
    ...
    DWORD winid;
    GetWindowThreadProcessId(hwnd1, &winid);
    if (pids.empty())
        return FALSE;
    ...
    for (DWORD pid : pids)
    {
        if (winid == pid)
        {
            std::wstring title(GetWindowTextLength(hwnd1) + 1, L'\0');
            GetWindowTextW(hwnd1, &title[0], title.size());
...
float lasttime = 0.f;
std::string display = "";
void displaycurrentspotifysong() 
{
    if (!config->misc.wm.showSpotify)
        return;

    if (memory->globalVars->realtime > lasttime + 5.f)
    {
        if (spotifywindow()) {
            std::string title2;
            if (char what[128]; WideCharToMultiByte(CP_UTF8, 0, ttl.c_str(), -1, what, 128, nullptr, nullptr))
                title2 = std::string(what);
            Misc::spotifytitle = wcsstr(ttl.c_str(), L"spotify") ? c_xor("Stopped / Paused") : title2;
            if (strstr(Misc::spotifytitle.c_str(), c_xor("Spotify Premium")))
                Misc::spotifytitle = c_xor("Stopped / Paused");
            if (strstr(Misc::spotifytitle.c_str(), c_xor("Spotify Free")))
                Misc::spotifytitle = c_xor("Stopped / Paused");
        }
        else
            Misc::spotifytitle = c_xor("Spotify Not Found");
...
static HRESULT __stdcall present(IDirect3DDevice9* device, const RECT* src, const RECT* dest, HWND windowOverride, const RGNDATA* dirtyRegion) noexcept
{
    ...
    if (const auto& displaySize = ImGui::GetIO().DisplaySize; displaySize.x > 0.0f && displaySize.y > 0.0f) {
        ...
        NadePrediction::draw(ImGui::GetBackgroundDrawList());
        ...
        Visuals::TaserRange(ImGui::GetBackgroundDrawList());
...
{
    //VMProtectBeginMutation("isConnected");
...
static int __fastcall sendDatagramHook(NetworkChannel* network, void* edx, void* datagram)
{
    ...
    if (!config->backtrack.enabled || !config->backtrack.fakeLatency || datagram || !interfaces->engine->isInGame())
...
static bool __stdcall createMove(float inputSampleTime, UserCmd* cmd, bool& sendPacket) noexcept
{
    ...
    Misc::autoStrafe(cmd, currentViewAngles);
    Misc::jumpBug(cmd);
    Misc::edgeBug(cmd, cmd->viewangles.y);
    Misc::ebdetections(cmd);
...
{
    if (std::uintptr_t(_ReturnAddress()) == memory->cameraThink && (config->visuals.thirdperson.enable || config->visuals.freeCam))
        return true;
...
static bool __stdcall shouldDrawFog() noexcept
{
    ...
    if (config->visuals.noFog && (std::uintptr_t)_ReturnAddress() == memory->shouldDrawFogReturnAddress)
        return false;
    else
        return true;
}
...
{
    if (config->visuals.viewmodelInScope && localPlayer && localPlayer->isAlive() && localPlayer->getActiveWeapon() && localPlayer->isScoped())
        return memory->viewmodelInScope;
...
static void __stdcall setDrawColor(int r, int g, int b, int a) noexcept
{
    if (config->visuals.noScopeOverlay && (std::uintptr_t(_ReturnAddress()) == memory->scopeDust || std::uintptr_t(_ReturnAddress()) == memory->scopeArc))
        a = 0;
    if (localPlayer && localPlayer->isAlive() && (config->visuals.scope.type == 2 || config->visuals.scope.type == 1 || config->visuals.scope.type == 3) && (std::uintptr_t(_ReturnAddress()) == memory->staticScopeLensClear || std::uintptr_t(_ReturnAddress()) == memory->staticScopeLensBlurry))
        a = 0;
...
{
    const auto zoomSensitivity = interfaces->cvar->findVar(skCrypt("zoom_sensitivity_ratio_mouse"));
    static auto zoomSensitivityBackUp = zoomSensitivity->getFloat();
    if (localPlayer)
    {
        //clip players
        bool thirdperson = config->visuals.thirdperson.enable && config->visuals.thirdperson.key.isActive() && localPlayer->isAlive();

        if (memory->input->isCameraInThirdPerson && thirdperson)
        {
            float distance = 0.f;
            ...
            distance = static_cast<float>(config->visuals.thirdperson.distance);
            inverse_angles.z = distance;
            Vector forward, right, up;
            Vector::AngleVectors(angles, forward, right, up);
            auto eye_pos = localPlayer->getEyePosition();
            auto offset = eye_pos + forward * -distance + right + up;
            TraceFilterWorldAndPropsOnly filter;            
            interfaces->engineTrace->traceRay1(Ray(eye_pos, offset, Vector(-16.0f, -16.0f, -16.0f), Vector(16.0f, 16.0f, 16.0f)), MASK_SOLID, filter, tr);
            angles.z = distance * tr.fraction;
    ...
    if (localPlayer && localPlayer->isAlive() && config->misc.fakeduck && config->misc.fakeduckKey.isActive() && localPlayer->flags() & 1)
        setup->origin = localPlayer->getAbsOrigin() + interfaces->gameMovement->getPlayerViewOffset(false);
...
static void __fastcall overrideMouseInput(ClientMode* thisptr, int edx, float* x, float* y)
{
    if (!interfaces->engine->isInGame() || !localPlayer)
        return hooks->clientMode.callOriginal<void, 23>(x,y);

    if (memory->globalVars->tickCount > Misc::detectdata.detecttick && memory->globalVars->tickCount <= Misc::detectdata.edgebugtick)
    {

        switch (config->misc.edgeBugLockType)
        {
        case 0:
            break;
        case 1:
            *x *= 1.f - config->misc.edgeBugLock;
            break;
        case 2:

            if (*x != 0.f)
            {
                float ratio = config->misc.edgeBugLock;

                float full_tick_amount = Misc::detectdata.edgebugtick - Misc::detectdata.detecttick;
                float ticks_left_to_eb = Misc::detectdata.edgebugtick - memory->globalVars->tickCount;
...
static bool __fastcall canUnduck(void* thisPointer, void* edx) noexcept
{
    ...
    static auto mp_solid_teammates = interfaces->cvar->findVar(skCrypt("mp_solid_teammates"));
...
static bool __fastcall dispatchUserMessage(void* thisPointer, void* edx, int messageType, int argument, int secondArgument, void* data) noexcept
{
    ...
    if (messageType == CS_UM_VoteStart)
        Misc::onVoteStart(data, secondArgument);
    if (messageType == CS_UM_VoteFailed)
        Misc::onVoteFailed();
...
static bool __fastcall postNetworkDataReceivedHook(void* thisPointer, void* edx, int commandsAcknowledged) noexcept
{
    ...
    static auto cl_showerror = interfaces->cvar->findVar(skCrypt("cl_showerror"));

    bool haderrors = false;

    // Store network data into post networking pristine state slot (slot 64)
    memory->saveData(thisPointer, skCrypt("PostNetworkDataReceived"), SLOT_ORIGINALDATA, PC_EVERYTHING);
...
void Hooks::install() noexcept
{
    ...
    **reinterpret_cast<decltype(reset)***>(memory->reset) = reset;*/
    ...
    mdlCache.init(interfaces->mdlCache);
    ...
    clientMode.hookAt(23, overrideMouseInput);
    ...
    svCheats.init(interfaces->cvar->findVar(skCrypt("sv_cheats")));
...
{
    interfaces->engine->clientCmdUnrestricted("quit");
    if constexpr (std::is_same_v<HookType, MinHook>) {
